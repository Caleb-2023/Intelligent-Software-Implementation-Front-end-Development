...
function updateAiWeatherContext() {
  const weatherSelect = document.getElementById('am-ai-weather')
  if (weatherSelect) {
    weatherSelect.value = state.selections.weather || 'cloudy'
  }
  updateAccountDashboard()
}
...
function updateAiResultsSummary(message = '') {
  ...
  results.innerHTML = `
    ...
    Weather: ${escapeHtml(state.selections.weather || 'cloudy')}
...
// Fill the single AI visual panel: show the returned outfit render when we have one,
// otherwise fall back to the canvas avatar visualization (local mode / no image).
async function renderAiRecommendationVisual(recommendation) {
  const canvas = document.getElementById('am-ai-canvas')
  const image = document.getElementById('am-ai-image')
  const imageUrl = resolveAssetUrl(recommendation.imageUrl || '')

  if (imageUrl && image) {
    if (canvas) canvas.hidden = true
    image.hidden = false
    image.alt = recommendation.headline || 'AI outfit recommendation'
    image.onerror = () => {
      // Remote image failed to load — revert to the canvas avatar visual.
      image.onerror = null
      image.hidden = true
      image.removeAttribute('src')
      if (canvas) canvas.hidden = false
      renderAiVisualFromCurrentPreview(recommendation).catch(() => {})
    }
    image.src = imageUrl
    return
  }
...
function buildRecommendationContext() {
  return {
    ...
    weatherLabel: state.selections.weather || 'cloudy',
...
async function requestRemoteRecommendation() {
  ...
  const response = await fetch(API_ENDPOINTS.recommendations, {
    ...
    body: JSON.stringify({
      ...
      weather: state.selections.weather || 'cloudy',
      style: '',
      gender: '',
    }),
  })
...
function applyAuthenticatedSession({ token, user }) {
  ...
  loadTryonHistory()
  loadUserFigure()
  document.dispatchEvent(new CustomEvent('am:auth-changed'))
}
...
async function loadTryonHistory() {
  ...
  try {
    ...
    if (resp.status === 401) {
      container.innerHTML = '<div class="am-empty-state">Session expired — please sign in again.</div>'
      return
    }
...
async function handleAiRecommendationRequest() {
  ...
  try {
    await renderAiRecommendationVisual(recommendation)
  } catch (error) {
    visualRenderError =
      error instanceof Error ? error.message : 'unknown error'
  }
...
async function loadUserFigure() {
  const token = getApiToken()
  if (!token) return

  try {
    const response = await fetch(API_ENDPOINTS.figures, {
      headers: { Authorization: `Bearer ${token}` },
    })
    ...
    const figureId = String(figure?._id || figure?.id || '')
    if (!figureId) return

    state.figureId = figureId

    // Load the avatar image so the preview and try-on pipeline are also ready
    const imageResp = await fetch(`${API_ENDPOINTS.figures}/${figureId}/image`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    ...
    ctx.fillText('Loaded from your account', 34, 72)
    ctx.restore()
    ...
    updateFigureStatus('Avatar loaded from your account.')
    updateAccountDashboard()
    ...
    // Best-effort — silently skip if figure cannot be loaded
  }
}
...
function bindAiStylistOverlay(stylingControls = {}) {
  ...
  weatherSelect?.addEventListener('change', (event) => {
    state.selections.weather = String(event.currentTarget.value || 'cloudy')
    updateAiStatus(`Weather set to ${state.selections.weather}.`)
    refreshAiContext()
  })
